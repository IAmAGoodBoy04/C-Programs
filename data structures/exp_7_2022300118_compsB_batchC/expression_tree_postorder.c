...
bool isOperator(char c) {
    return (c == '+' || c == '-' || c == '*' || c == '/');//This function determines whether the given character is an operator or not
}
...
// TODO: To be completed
float evaluate_ET(ExprTreeNode* root){//this function evaluates the value of the expression tree, the method used is taken from the ppt provided on moodle.
...
int main(){
    ...
    str[strlen(str)-1]='\0';//replace the '\n' with a '\0', since we do not need a newline character at the end of our string.
